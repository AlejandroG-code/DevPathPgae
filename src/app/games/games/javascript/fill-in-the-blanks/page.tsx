// src/app/games/fill-in-the-blanks/page.tsx
'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allFillInTheBlanksQuestions, FillInTheBlanksQuestion } from './data'; // Asegúrate de que 'data' sea la ruta correcta

// Define cuántas preguntas aleatorias mostrar por juego
const QUESTIONS_PER_GAME = 10;

const FillInTheBlanksPage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<FillInTheBlanksQuestion[]>([]);
  ...
  const selectRandomQuestions = useCallback(() => {
    ...
    if (allFillInTheBlanksQuestions.length <= QUESTIONS_PER_GAME) {
      return [...allFillInTheBlanksQuestions].sort(() => 0.5 - Math.random());
    }
    const shuffled = [...allFillInTheBlanksQuestions].sort(() => 0.5 - Math.random());
  ...
  const handleSubmitAnswer = () => {
    ...
    if (userAnswer.trim().toLowerCase() === currentQuestion.correctAnswer.toLowerCase()) {
      ...
      setFeedback('Incorrect. Keep trying or reveal the explanation.');
    }
  ...
  const handleNextQuestion = () => {
    setFeedback(null);
    setShowExplanation(false);
    setUserAnswer(''); // Limpia la respuesta del usuario para la siguiente pregunta
          ...
          <div>
            ...
            <div className="bg-gray-900/50 p-4 rounded-md mb-6 border border-gray-700">
              <pre className="text-green-300 whitespace-pre-wrap font-mono text-sm">
                ...
                <input
                  type="text"
                  className="inline-block w-32 bg-gray-700 text-white border-b-2 border-vibrant-teal focus:outline-none px-1 py-0.5 text-center"
                  value={userAnswer}
