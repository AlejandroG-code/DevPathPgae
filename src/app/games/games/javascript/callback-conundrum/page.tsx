// src/app/games/javascript/callback-conundrum/page.tsx
'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allCallbackConundrumQuestions, CallbackConundrumQuestion } from './data';

const QUESTIONS_PER_GAME = 5;

const CallbackConundrumPage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<CallbackConundrumQuestion[]>([]);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  const selectRandomQuestions = useCallback(() => {
    if (allCallbackConundrumQuestions.length <= QUESTIONS_PER_GAME) {
      return [...allCallbackConundrumQuestions].sort(() => 0.5 - Math.random());
    }
    const shuffled = [...allCallbackConundrumQuestions].sort(() => 0.5 - Math.random());
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-[calc(100vh-64px)] text-white bg-gray-900">
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-vibrant-teal text-center drop-shadow-md">
        JavaScript: Callback Conundrum
      </h1>
      <p className="text-gray-200 mb-8 text-center text-lg max-w-3xl mx-auto">
        Predict the console output of asynchronous JavaScript code snippets.
      </p>
          ...
          <div>
            ...
            <div className="bg-gray-900/50 p-4 rounded-md mb-6 border border-gray-700">
              <pre className="text-blue-300 whitespace-pre-wrap font-mono text-sm">
                {currentQuestion?.codeSnippet.trim()}
              </pre>
            </div>
...
export default CallbackConundrumPage;