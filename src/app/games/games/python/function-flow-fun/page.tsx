// src/app/games/python/function-flow-fun/page.tsx
'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allFunctionFlowFunQuestions, FunctionFlowFunQuestion } from './data';

const QUESTIONS_PER_GAME = 5;

const FunctionFlowFunPage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<FunctionFlowFunQuestion[]>([]);
  const [selectedOption, setSelectedOption] = useState<string | null>(null);

  const selectRandomQuestions = useCallback(() => {
    if (allFunctionFlowFunQuestions.length <= QUESTIONS_PER_GAME) {
      return [...allFunctionFlowFunQuestions].sort(() => 0.5 - Math.random());
    }
    const shuffled = [...allFunctionFlowFunQuestions].sort(() => 0.5 - Math.random());
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-[calc(100vh-64px)] text-white bg-gray-900">
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-vibrant-teal text-center drop-shadow-md">
        Python: Function Flow Fun
      </h1>
      <p className="text-gray-200 mb-8 text-center text-lg max-w-3xl mx-auto">
        Trace the execution of Python functions and predict their output.
      </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {currentQuestion?.options.map((option, index) => (
                ...
                >
                  {option.includes('\n') ? ( // Render multiline output with pre-wrap
...
export default FunctionFlowFunPage;