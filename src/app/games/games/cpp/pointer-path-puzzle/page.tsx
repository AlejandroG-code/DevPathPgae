// src/app/games/cpp/pointer-path-puzzle/page.tsx
'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allPointerPathPuzzleQuestions, PointerPuzzleQuestion } from './data';

const QUESTIONS_PER_GAME = 5;

const normalizeText = (text: string) => text.trim().replace(/\s+/g, ' ').toLowerCase();

const PointerPathPuzzlePage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<PointerPuzzleQuestion[]>([]);

  const selectRandomQuestions = useCallback(() => {
    if (allPointerPathPuzzleQuestions.length === 0) return [];
    const shuffled = [...allPointerPathPuzzleQuestions].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(QUESTIONS_PER_GAME, allPointerPathPuzzleQuestions.length));
  }, []);
  ...
  const handleSubmitAnswer = () => {
    ...
    if (normalizeText(userAnswer) === normalizeText(currentQuestion.correctAnswer)) {
      setFeedback('Correct! You navigated the pointer path.');
  ...
  if (sessionQuestions.length === 0 && !gameOver) {
    return (
      <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
        ...
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-cyan-400 mx-auto mt-5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
      ...
      <button
        ...
        className="mb-6 text-cyan-400 hover:text-cyan-300 transition-colors duration-200 flex items-center text-lg font-medium self-start"
      >
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-cyan-400 text-center drop-shadow-md">
        C++: Pointer Path Puzzle
      </h1>
      <p className="text-gray-300 mb-8 text-center text-lg max-w-3xl mx-auto">
        Navigate memory and pointers in C++.
      </p>
          ...
          <div>
            ...
            <div className="bg-gray-700/50 p-4 rounded-md mb-6 border border-gray-600">
              <pre className="text-amber-300 whitespace-pre-wrap font-mono text-sm">
                ...
                <input
                  type="text"
                  className="inline-block w-48 bg-gray-900 text-white border-b-2 border-cyan-400 focus:outline-none px-1 py-0.5 text-center font-mono"
                  value={userAnswer}
            ...
            <div className="flex flex-col gap-4 mb-6">
              <button
                ...
                className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-3 px-6 rounded-lg text-lg transition-colors duration-200 shadow-md"
            ...
            {feedback && (
              <div className="mt-4 p-4 rounded-md bg-gray-700 border border-gray-600">
                ...
                {showExplanation && (
                  <>
                    <h4 className="text-md font-semibold text-cyan-400 mt-3 mb-1">Explanation:</h4>
...
export default PointerPathPuzzlePage;