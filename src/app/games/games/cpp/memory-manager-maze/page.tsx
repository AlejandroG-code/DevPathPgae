// src/app/games/cpp/memory-manager-maze/page.tsx
'use client';

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allMemoryManagerMazeQuestions, MemoryMazeQuestion } from './data';

const QUESTIONS_PER_GAME = 5;

const normalizeText = (text: string) => text.trim().replace(/\s+/g, ' ').toLowerCase();

const MemoryManagerMazePage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<MemoryMazeQuestion[]>([]);

  const selectRandomQuestions = useCallback(() => {
    if (allMemoryManagerMazeQuestions.length === 0) return [];
    const shuffled = [...allMemoryManagerMazeQuestions].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(QUESTIONS_PER_GAME, allMemoryManagerMazeQuestions.length));
  }, []);
  ...
  const handleSubmitAnswer = () => {
    ...
    if (normalizeText(userAnswer) === normalizeText(currentQuestion.correctAnswer)) {
      setFeedback('Correct! You managed the memory.');
  ...
  if (sessionQuestions.length === 0 && !gameOver) {
    return (
      <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
        ...
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-orange-400 mx-auto mt-5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
      ...
      <button
        ...
        className="mb-6 text-orange-400 hover:text-orange-300 transition-colors duration-200 flex items-center text-lg font-medium self-start"
      >
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-orange-400 text-center drop-shadow-md">
        C++: Memory Manager Maze
      </h1>
      <p className="text-gray-300 mb-8 text-center text-lg max-w-3xl mx-auto">
        Identify and fix memory errors in C++.
      </p>
      ...
      <div className="bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-700 w-full max-w-3xl">
        {!gameOver ? (
          <div>
            ...
            <div className="bg-gray-700/50 p-4 rounded-md mb-6 border border-gray-600">
              <pre className="text-amber-300 whitespace-pre-wrap font-mono text-sm">
                ...
                <input
                  type="text"
                  className="inline-block w-48 bg-gray-900 text-white border-b-2 border-orange-400 focus:outline-none px-1 py-0.5 text-center font-mono"
                  value={userAnswer}
            ...
            <div className="flex flex-col gap-4 mb-6">
              <button
                ...
                className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-6 rounded-lg text-lg transition-colors duration-200 shadow-md"
            ...
            {feedback && (
              <div className="mt-4 p-4 rounded-md bg-gray-700 border border-gray-600">
                ...
                {showExplanation && (
                  <>
                    <h4 className="text-md font-semibold text-orange-400 mt-3 mb-1">Explanation:</h4>
          ...
          <div className="text-center py-10">
            <h2 className="text-3xl font-bold text-orange-400 mb-4">Game Over!</h2>
...
export default MemoryManagerMazePage;