// src/app/games/arduino/pin-power-up/page.tsx
'use client'; // This directive is important for client-side functionality

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allPinPowerUpQuestions, PinPowerUpQuestion } from './data'; // Correct relative import
...
// The main component for the Pin Power-Up game
const PinPowerUpPage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<PinPowerUpQuestion[]>([]);

  // Function to select random questions for a new game session
  const selectRandomQuestions = useCallback(() => {
    if (allPinPowerUpQuestions.length === 0) return [];
    const shuffled = [...allPinPowerUpQuestions].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(QUESTIONS_PER_GAME, allPinPowerUpQuestions.length));
  }, []);
  ...
  const handleSubmitAnswer = () => {
    ...
    if (normalizeText(userAnswer) === normalizeText(currentQuestion.correctAnswer)) {
      setFeedback('Correct! You’ve mastered pin control.');
  ...
  if (sessionQuestions.length === 0 && !gameOver) {
    return (
      <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
        ...
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-400 mx-auto mt-5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
      ...
      <button
        ...
        className="mb-6 text-green-400 hover:text-green-300 transition-colors duration-200 flex items-center text-lg font-medium self-start"
      >
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-green-400 text-center drop-shadow-md">
        Arduino: Pin Power-Up
      </h1>
      <p className="text-gray-300 mb-8 text-center text-lg max-w-3xl mx-auto">
        Learn to control Arduino pins for LEDs and basic I/O.
      </p>
      ...
      <div className="bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-700 w-full max-w-3xl">
        {!gameOver ? (
          <div>
            ...
            <div className="bg-gray-700/50 p-4 rounded-md mb-6 border border-gray-600">
              <pre className="text-orange-300 whitespace-pre-wrap font-mono text-sm">
                ...
                <input
                  type="text"
                  className="inline-block w-48 bg-gray-900 text-white border-b-2 border-green-400 focus:outline-none px-1 py-0.5 text-center font-mono"
                  value={userAnswer}
            ...
            {feedback && (
              <div className="mt-4 p-4 rounded-md bg-gray-700 border border-gray-600">
                ...
                {showExplanation && (
                  <>
                    <h4 className="text-md font-semibold text-green-400 mt-3 mb-1">Explanation:</h4>
          ...
          <div className="text-center py-10">
            <h2 className="text-3xl font-bold text-green-400 mb-4">Game Over!</h2>
...
export default PinPowerUpPage; // Ensure this is the correct component name