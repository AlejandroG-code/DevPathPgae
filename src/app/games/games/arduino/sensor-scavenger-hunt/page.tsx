// src/app/games/arduino/sensor-scavenger-hunt/page.tsx
'use client'; // This directive is important for client-side functionality

import React, { useState, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { allSensorScavengerHuntQuestions, SensorScavengerHuntQuestion } from './data'; // Correct relative import
...
// The main component for the Sensor Scavenger Hunt game
const SensorScavengerHuntPage: React.FC = () => {
  ...
  const [sessionQuestions, setSessionQuestions] = useState<SensorScavengerHuntQuestion[]>([]);

  // Function to select random questions for a new game session
  const selectRandomQuestions = useCallback(() => {
    if (allSensorScavengerHuntQuestions.length === 0) return [];
    const shuffled = [...allSensorScavengerHuntQuestions].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(QUESTIONS_PER_GAME, allSensorScavengerHuntQuestions.length));
  }, []);
  ...
  const handleSubmitAnswer = () => {
    ...
    if (normalizeText(userAnswer) === normalizeText(currentQuestion.correctAnswer)) {
      setFeedback('Correct! You’ve found the sensor solution.');
  ...
  if (sessionQuestions.length === 0 && !gameOver) {
    return (
      <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
        ...
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500 mx-auto mt-5"></div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center p-8 min-h-screen bg-gray-900 text-white">
      ...
      <button
        ...
        className="mb-6 text-purple-400 hover:text-purple-300 transition-colors duration-200 flex items-center text-lg font-medium self-start"
      >
      ...
      <h1 className="text-4xl font-extrabold mb-4 text-purple-400 text-center drop-shadow-md">
        Arduino: Sensor Scavenger Hunt
      </h1>
      <p className="text-gray-300 mb-8 text-center text-lg max-w-3xl mx-auto">
        Interact with various sensors using Arduino code.
      </p>
          ...
          <div>
            ...
            <div className="bg-gray-700/50 p-4 rounded-md mb-6 border border-gray-600">
              <pre className="text-orange-300 whitespace-pre-wrap font-mono text-sm">
                ...
                <input
                  type="text"
                  className="inline-block w-48 bg-gray-900 text-white border-b-2 border-purple-400 focus:outline-none px-1 py-0.5 text-center font-mono"
                  value={userAnswer}
            ...
            {feedback && (
              <div className="mt-4 p-4 rounded-md bg-gray-700 border border-gray-600">
                ...
                {showExplanation && (
                  <>
                    <h4 className="text-md font-semibold text-purple-400 mt-3 mb-1">Explanation:</h4>
...
export default SensorScavengerHuntPage; // Ensure this is the correct component name