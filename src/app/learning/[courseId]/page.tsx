    ...
    import { notFound } from 'next/navigation'; // Correcto: no useParams aquí
    import Link from 'next/link';
    import { motion } from 'framer-motion';

    // Asegúrate de que esta ruta sea correcta para tu `courses_meta.json`
    // Si está en `public/data`, esta ruta está bien:
    import coursesMetadata from '../../../../public/data/courses_meta.json';
    // Si está en `src/data`, debería ser:
    // import coursesMetadata from '../../../../data/courses_meta.json';
    ...
    // INTERFAZ DE PROPS PARA LA PÁGINA: ESTO ES CRUCIAL
    interface CourseDetailPageProps {
      params: {
        courseId: string; // Esto DEBE ser 'string', no una Promesa
      };
      // Aquí también podrían ir searchParams si los usaras:
      // searchParams?: { [key: string]: string | string[] | undefined };
    }

    const CourseDetailPage: React.FC<CourseDetailPageProps> = ({ params }) => {
      // Obtenemos courseId directamente de las props 'params' que Next.js inyecta
      ...
      // 2. Si no se encuentra el curso, activar 404
      if (!course) {
