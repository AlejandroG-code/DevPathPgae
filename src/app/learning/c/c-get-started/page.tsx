...
export default function CGetStartedPage({ params }: LessonPageProps) {
  useEffect(() => {
    ...
    // 2. Load PrismJS core script
    const scriptCore = document.createElement('script');
    scriptCore.src = 'https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/prism.min.js';
    scriptCore.async = true;

    // 3. Once core is loaded, load specific language components (C and Bash)
    scriptCore.onload = () => {
      ...
      scriptCLang.onload = () => {
        ...
        scriptBashLang.onload = () => {
          // Ensure Prism is globally available before highlighting
          if (window.Prism) {
            window.Prism.highlightAll(); // Trigger highlighting for all code blocks
          }
        };
    ...
    // Cleanup function to remove CSS link when component unmounts
    return () => {
      if (document.head.contains(link)) {
        document.head.removeChild(link);
      }
      // Scripts are generally left in the DOM as they execute once and don't conflict
      // unless specifically designed to. For simplicity, we don't remove them here.
    };
  ...
  return (
    <div className="flex"> {/* Contenedor flex para la barra lateral y el contenido */}
      ...
      <ReactMarkdown
        rehypePlugins={[rehypeRaw]} // Required to process custom HTML within Markdown
        components={{
          // Custom rendering for code blocks
          code: ({ inline, className, children, ...props }: React.HTMLAttributes<HTMLElement> & { inline?: boolean }) => {
            const match = /language-(\w+)/.exec(className || '');
            const lang = match ? match[1] : 'markup'; // Extract language (e.g., 'c', 'bash') or default to 'markup'

            return !inline ? (
              // For block code, create a <pre><code>. PrismJS needs 'language-XYZ' class on <code> tag.
              ...
              // Styling for inline code snippets (e.g., `printf()`)
              <code className="bg-gray-700 text-vibrant-teal px-1 py-0.5 rounded-md text-xs" {...props}>
                {children}
              </code>
            );
          },
          // Custom Tailwind CSS classes for other Markdown elements
          ...
          // Standard Markdown lists are handled here
